...
w = {
    'conv1': tf.Variable(tf.truncated_normal(shape=[5, 5, 1, 32 * m], stddev=0.1), name='w_c1'),
    # 对应filter 第一二参数值得卷积核尺寸大小，即patch，第三个参数是图像通道数，第四个参数是卷积核的数目，代表会出现多少个卷积特征图像
    'conv2': tf.Variable(tf.truncated_normal(shape=[5, 5, 32 * m, 64 * m], stddev=0.1), name='w_c2'),
    'fc1': tf.Variable(tf.truncated_normal(shape=[7 * 7 * 64 * m, 1024], stddev=0.1), name='w_f1'),
    # 只有一行7*7*64个数据的卷积，第二个参数代表卷积个数共1024个 7=28/2/2
    'fc2': tf.Variable(tf.truncated_normal(shape=[1024, 10], stddev=0.1), name='w_f2'),
}

b = {
    'conv1': tf.Variable(tf.constant(0.1, shape=[32 * m]), name='b_c1'),
    'conv2': tf.Variable(tf.constant(0.1, shape=[64 * m]), name='b_c2'),
...
def train(mnist, isTrain=True):
    ...
    optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss=cross_entropy, global_step=global_step)
    ...
    with tf.Session() as sess:
        ...
        if isTrain:
            ...
            while step < iteration:
                ...
                step = sess.run(global_step)  # 104行 会读取原来的global_step 或 0
                if step % display_step == 0:
                    val_accuracy = sess.run(accuracy, feed_dict=val_data)  # 5000个验证集丢进去
                    print('[{}] [{:.2f}] [{:.4f}]'.format(step, loss, val_accuracy))
                    saver.save(sess, './model2/model.ckpt', global_step=step)
            print('*' * 10)
            ...
            if ckpt and ckpt.model_checkpoint_path:
                saver.restore(sess, ckpt.model_checkpoint_path)
                # tes_data = {x: mnist.test.images[:5000, :], y: mnist.test.labels[:5000, :], keep_prob: 1.0}
                # final_accuracy = sess.run(accuracy, feed_dict=tes_data)
                # print('the test accuracy is:{:.4f}'.format(final_accuracy))
