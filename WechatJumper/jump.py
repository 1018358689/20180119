...
def isSimilar(color1, color2, mistake=10):
    return math.sqrt(
        (color1[0] - color2[0]) ** 2 + (color1[1] - color2[1]) ** 2 + (color1[2] - color2[2]) ** 2) <= mistake * 3
    ...
    if doll_pos == None:
        ...
        while True:
            if isSimilar(DOLL_FOOT_COLOR, img.getpixel((doll_pos[0] + 1, doll_pos[1]))):
                ...
                if doll_pos[0] >= doll_right_foot_pos[0] and isSimilar(DOLL_FOOT_COLOR,
                                                                       img.getpixel((doll_pos[0], doll_pos[1] - 1))):
        ...
        while isSimilar(img.getpixel(doll_left_foot_pos),
                        img.getpixel((doll_left_foot_pos[0] - 1, doll_left_foot_pos[1]))):
...
def screenShot():
    ...
    COUNT = COUNT + 1
    os.system("{} shell /system/bin/screencap -p /sdcard/screencap.png".format(adb_bin))
    os.system("{} pull /sdcard/screencap.png {}".format(adb_bin, SCREENSHOT_PATH))


def jump(time):
    os.system("{} shell input swipe 0 0 0 0 {}".format(adb_bin, str(int(time + 0.5))))
